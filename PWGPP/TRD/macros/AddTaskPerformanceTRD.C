...
//
// gROOT->LoadMacro("$ALICE_PHYSICS/PWGPP/TPC/macros/AddTaskPerformanceTRD.C");
// AliPerformanceTask *tpcQA = AddTaskPerformanceTRD("kFALSE","kTRUE","kFALSE","triggerClass",kFALSE);
//
// Output:
...
//used in QAtrain_duo with
//AddTaskPerformanceTPCdEdxQA(kTRUE, kTRUE, kFALSE, 0, kFALSE, kFALSE, kTRUE, kTRUE); //offline
//AddTaskPerformanceTPCdEdxQA(kTRUE, kFALSE, kFALSE, 0, kTRUE, kFALSE, kTRUE, kTRUE); //HLT
/*
AliPerformanceTRD* AddTaskPerformanceTRD(Bool_t bUseHLT, Bool_t isMC, UInt_t kTriggerMask)
{
  const Char_t *taskName = bUseHLT ? "TRD_HLT" : "TRD_offline";

  //
  // Add AliPerformanceTask with TRD performance components
  //
  AliAnalysisManager *mgr = AliAnalysisManager::GetAnalysisManager();
  if(!mgr) {
    Error("AddTaskPerformanceTRD","AliAnalysisManager not set!");
    return NULL;
  }

  AliVEventHandler *handler = mgr->GetInputEventHandler();
  if(!handler) {
    Error("AddTaskPerformanceTRD", "input event handler needed!");
    return NULL;
  }
  const TString type = handler->GetDataType();
  if (!type.Contains("ESD")) {
    Error("AddTaskPerformanceTRD", "ESD input handler needed!");
    return NULL;
  }
  ...
  //
  if (bUseHLT) {
    AliESDInputHandler *esdH = dynamic_cast<AliESDInputHandler*>(handler);
    esdH->SetReadHLT();
  }
  ...
  // Create task
  //
  AliPerformanceTask *task = new AliPerformanceTask("PerformanceQA", Form("%s Performance",taskName));
  if (!task) {
    Error("AddTaskPerformanceTRD", Form("%s performance task cannot be created!",taskName));
    return NULL;
  }
...
AliPerformanceTRD* AddTaskPerformanceTRD(Bool_t bUseHLT, Bool_t isMC=kFALSE, UInt_t kTriggerMask=0)
{
  const Char_t *taskName = bUseHLT ? "TRD_HLT" : "TRD_offline";
  ...
  fileName  += ":AliPerformanceTRD";  //  create  a  subfolder  in  the  file
  //  now  we  create  an  instance  of  your  task
  AliPerformanceTRD *task = new  AliPerformanceTRD(taskName);
  //offline or HLT?
  task->SetUseHLT(bUseHLT);
  // in MC no trigger simulated, otherwise only use
  if(!isMC){task->SelectCollisionCandidates(kTriggerMask);}
  //  add  your  task  to  the  manager
  mgr ->AddTask(task);
  //  connect  the  manager  to  your  task
  mgr ->ConnectInput(task ,0,mgr ->GetCommonInputContainer ());
  //  same  for  the  output
  mgr ->ConnectOutput(task ,1,mgr ->CreateContainer(Form("AliPerformance_%s",taskName), TList::Class (), AliAnalysisManager::kOutputContainer , fileName.Data()));
